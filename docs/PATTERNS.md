...
- name: Run Claude
  ...
  with:
    ...
    prompt: ${{ steps.prompt.outputs.content }}
    claude_args: >-
      --allowedTools "Edit,Write,Read,Glob,Grep,Bash(git log:*),Bash(git diff:*),Bash(git status:*),Bash(gh pr:*)"
      --model ${{ vars.AI_MODEL_EXAMPLE || vars.AI_MODEL }}
```
...
This violates the [Concurrency Pattern](#concurrency-pattern) but is outside our control — the lock file is owned by the compiler.
...
gh run list --workflow=ai-moderator.lock.yml --repo JacobPEvans/ai-workflows --limit 3 \
